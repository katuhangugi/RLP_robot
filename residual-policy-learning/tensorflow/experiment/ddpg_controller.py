...
class DDPG(object):
    ...
    #             obs['observation'] = obs['observation'][int(o.shape[1] / 2):]
    #         if 'Hook' in self.info['env_name']:
    #             control_action = get_hook_control(obs)
    #         elif 'MPCPush' in self.info['env_name']:
    #             mpc_obs = {'observation': obs['observation'][:25], 'desired_goal': g[i, :]}
    #             sim_state = obs['observation'][25:]
    #             control_action = self.hardcoded_controller.act(
    #                 mpc_obs, {'sim_state': {'qpos': sim_state[:22], 'qvel': sim_state[22:]}}
    #             )
    #         elif 'NoisyPusher' in self.info['env_name']:
    #             control_action = self.hardcoded_controller(obs)
    #         elif 'Pusher' in self.info['env_name']:
    #             control_action = self.hardcoded_controller(obs)
    #         elif 'Push' in self.info['env_name']:
    #             control_action = get_push_control(obs)
    #         elif 'PickAndPlace' in self.info['env_name']:
    #             control_action = get_pick_and_place_control(obs)
    #             if not isinstance(control_action, np.ndarray):
    #                 raise TypeError(f"Control action is not a NumPy array. Got {type(control_action)} instead.")
    #             push_controls.append(self._pad_action(control_action))
    #     return np.stack(push_controls)
    
    # 尝试修改动作空间的维度
    # def _pad_action(self, action, target_dim=4):
    #     if not isinstance(action, np.ndarray):
    #         raise TypeError(f"Expected action to be a NumPy array, but got {type(action)} instead.")
    #     if action.shape[0] < target_dim:
    #         padded_action = np.zeros(target_dim)
    ...
    def _controller_action(self, o, g):
        """
        Generates control actions for the given observations and goals.

        Args:
            o (np.ndarray): Observations.
            g (np.ndarray): Goals.

        Returns:
            np.ndarray: Computed control actions.
        """
        push_controls = []
        obs = {'observation': None}
        ...
        for i in range(o.shape[0]):
            ...
            if 'Hook' in self.info['env_name']:
                ...
                control_action = get_pick_and_place_control(obs)
            elif 'UR5ePickAndPlace' in self.info['env_name']:
                control_action = get_pick_and_place_control(obs)

            # 确保 control_action 是 NumPy 数组
            if not isinstance(control_action, np.ndarray):
                raise TypeError(f"Control action is not a NumPy array. Got {type(control_action)} instead.")
    ...
    def _pad_action(self, action, target_dim=None):
        """
        Pads or truncates the action array to match the required dimension.

        Args:
            action (np.ndarray): The original action array.
            target_dim (int, optional): The target dimension of the action. If None, uses self.dimu.

        Returns:
            np.ndarray: The action array with correct dimensions.
        """
        if not isinstance(action, np.ndarray):
            raise TypeError(f"Expected action to be a NumPy array, but got {type(action)} instead.")
        ...
        if target_dim is None:
            if hasattr(self, "dimu"):
                target_dim = self.dimu
            else:
                raise ValueError("target_dim is not specified, and self.dimu is not available.")

        # 进行填充或裁剪
        if action.shape[0] < target_dim:
            padded_action = np.zeros(target_dim, dtype=action.dtype)
            padded_action[:action.shape[0]] = action
            return padded_action
        elif action.shape[0] > target_dim:
            return action[:target_dim]  # 截断过大的动作
        return action  # 已经匹配，无需修改
    ...
    def _create_network(self, reuse=False):
        ...
        if GPU_OPTI:
            ...
            Q_grads_tf = tf.compat.v1.gradients(self.Q_loss_tf, self._vars('main/Q'))
            pi_grads_tf = tf.compat.v1.gradients(self.pi_loss_tf, self._vars('main/pi'))
