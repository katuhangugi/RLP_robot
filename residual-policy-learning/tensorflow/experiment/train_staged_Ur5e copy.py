...
def launch(
    ...
    # 在logdir路径中添加时间戳
    logdir = os.path.join(logdir, f"{env}_{timestamp}")
    # 调用日志记录器的configure()方法，返回日志目录。
    # logger.configure()
    ...
    # Prepare params. 并行来源自json文件中的配置
...
@click.option('--env', type=str, default='UR5ePickAndPlace-v2', help='the name of the OpenAI Gym environment that you want to train on')
# @click.option('--logdir', type=str, default="/home/joe/Work/RPL_local/logs/seed0/FetchPickAndPlace-v1", help='the path to where logs and policy pickles should go. If not specified, creates a folder in /tmp/')
@click.option('--logdir', type=str, default="/home/joe/Work/RPL_local/logs/seed0/UR5ePickAndPlace-v2", help='the path to where logs and policy pickles should go. If not specified, creates a folder in /tmp/')
...
@click.option('--config_path', type=str, default="/home/joe/Work/RPL_local/residual-policy-learning/tensorflow/experiment/configs/pickandplace.json", help='path to json config file')
@click.option('--random_eps', type=float, default="0.3", help='random_eps if using expert explore baseline')
@click.option('--controller_prop', type=float, default="0.8", help='alpha if using expert explore baseline')
def main(**kwargs):
    # 它接受一个名为kwargs的关键字参数，表示所有的命令行选项。
    override_params = {}
    # 在函数内部，创建了一个空字典override_params。
    # 如果random_eps和controller_prop不是None，就将它们添加到override_params中。然后，将这个字典添加到kwargs中，并删除random_eps和controller_prop。
    ...
    # launch函数接受了一个名为kwargs的关键字参数，表示所有的命令行选项。
    kwargs['override_params'] = override_params # 将override_params添加到kwargs中。
    # pop() 函数用于删除列表中的一个元素（默认最后一个元素），并返回该元素的值。
    kwargs.pop('random_eps',None) # 此处的None是pop()函数的默认参数，表示如果没有指定要删除的元素，则默认删除最后一个元素。
